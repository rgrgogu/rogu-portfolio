...
const AdminInquiryDeleteModal = ({ id, setMsg }) => {
  const { dispatch } = useInquirysContext();

  const handleClick = async (e) => {
    const response = await fetch(`${API_LINK}/inquiry/` + id, {
      method: "DELETE",
    });
    ...
    if (response.ok) {
      ...
      HSOverlay.close(document.getElementById("admin-deleteInquiry"));
      dispatch({ type: "DELETE_INQUIRY", payload: json });
    }
  };

  return (
    <div
      id="admin-deleteInquiry"
        ...
        <div className="relative flex flex-col bg-white shadow-lg rounded-xl dark:bg-gray-800">
          ...
          <div className="p-4 sm:p-10 text-center overflow-y-auto">
            ...
            <p className="text-gray-500">
              Are you sure you would like to delete this inquiry?
            </p>
            <div className="mt-6 flex justify-center gap-x-4">
              <button
                onClick={handleClick}
                className="py-2.5 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-red-500 text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800"
              >
                Proceed
              </button>
              <button
...
export default AdminInquiryDeleteModal;
