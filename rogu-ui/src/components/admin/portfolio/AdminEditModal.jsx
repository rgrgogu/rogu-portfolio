...
const AdminEditModal = ({id, setMsg}) => {
  ...
  useEffect(() => {
    const fetchWorkouts = async () => {
      ...
      if (response.ok) {
        var fetchDate = json.date.substr(0, 10);
        const {title, category, description, image, link} = json;
        setForm({ title, category, date: fetchDate, description, image, link });
        
        var output = document.getElementById("editImg");
        output.src = json.image.url;
      }
    };
  ...
  const handleClick = async (e) => {
    ...
    if (response.ok) {
      setError(null);
      setMsg("Portfolio succesfully edited!");
      setEmptyFields([]);
      dispatch({ type: "UPDATE_PROJECT", payload: json });
      HSOverlay.close(document.getElementById("admin-edit"));
    }
  };
