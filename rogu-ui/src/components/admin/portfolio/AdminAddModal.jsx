...
import AdminAddModalForm from "./AdminAddModalForm";
import ErrorAlert from "../../client/global/ErrorAlert";
import { useProjectsContext } from "../../../hooks/useProjectsContext";

const AdminAddModal = ({ setSuccess }) => {
  ...
  const handleClick = async (e) => {
    ...
    const response = await fetch(`${API_LINK}/projects/`, {
      method: "POST",
      body: formData,
    });
    ...
    if (response.ok) {
      ...
      setImage();
      setError(null);
      setSuccess("New Portfolio added!");
      setEmptyFields([]);
      dispatch({ type: "CREATE_PROJECT", payload: json });
      HSOverlay.close(document.getElementById("admin-add"));
    }
  };
      ...
      <div className="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto h-[calc(100%-3.5rem)]">
        <div className="max-h-full overflow-hidden flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]">
          ...
          <div className="p-4 overflow-y-auto">
            <div className="mb-5">{error && <ErrorAlert msg={error} />}</div>
            <AdminAddModalForm
              form={form}
              image={image}
